...
from sim_pencil_game_alpha_go_zero.players.RobotArmPlayer import RobotArmPlayer
...
def main():
    ...
    p1 = RobotArmPlayer(game=game, driver=NNPlayer(game, args, MODEL_FOLDER, MODEL_NAME))
    # p1 = MinimaxPlayer(game, max_depth=10, player=1)
    p2 = HumanPlayer(game, player=-1)
    # p2 = RandomPlayer(game, player=-1)
    # p2 = MinimaxPlayer(game, max_depth=5, player=-1)
    arena = Arena(p1.play, p2.play, game, SimGameState.display)

    p1_wins, p2_wins, draws = arena.playGames(20, verbose=True)
